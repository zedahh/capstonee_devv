...
<title>IneSight | User Management</title>
...
<style>
  ...
  .bhms-sidebar-brand img.brand-logo { height: 42px; width: 42px; object-fit: cover; border-radius: 50%; padding: 2px; }
  .bhms-brand-title { display: block; font-weight: 600; font-size: 0.95rem; }
  .bhms-brand-sub { display: block; font-size: 0.78rem; opacity: 0.85; }
  ...
  .bhms-nav-link i { width: 18px; text-align: center; }
  ...
  .bhms-main { flex: 1 1 auto; margin-left: var(--bhms-sidebar-width); display: flex; flex-direction: column; min-height: 100vh; }
  .bhms-topbar {
  ...
  .bhms-menu-btn { display: none; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: var(--bhms-radius-sm); color: var(--bhms-blue-dark); cursor: pointer; }
    .bhms-topbar-title { font-weight: 600; font-size: 1.02rem; color: var(--bhms-gray-800); flex: 1 1 auto; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }t-weight: 600; flex: 1 1 auto; }
  .bhms-topbar-user { display: flex; align-items: center; gap: 0.5rem; font-size: 0.85rem; color: var(--bhms-gray-600); }
  .bhms-topbar-user .bhms-role-pill { font-size: 0.68rem; text-transform: uppercase; background: var(--bhms-blue-light); color: var(--bhms-blue-dark); padding: 0.15rem 0.55rem; border-radius: 999px; font-weight: 600; }
  ...
  .btn-outline-warning { color: #8a5a12; border-color: #E0932E; }
  .btn-outline-success { color: var(--bhms-success-darker); border-color: var(--bhms-success); }
  ...
  .form-label { font-weight: 500; font-size: 0.85rem; color: var(--bhms-gray-600); }
  .form-control, .form-select { border-radius: 10px; border: 1px solid var(--bhms-gray-300); padding: 0.55rem 0.9rem; font-size: 0.9rem; }
  ...
  .bhms-content .container > .d-flex.justify-content-between.align-items-center.mb-4 {
    background: rgba(255,255,255,0.75); border-radius: var(--bhms-radius-lg); padding: 1.1rem 1.4rem; box-shadow: var(--bhms-shadow-sm);
  }
  .users-table-card { background: #fff; border-radius: var(--bhms-radius-lg); box-shadow: var(--bhms-shadow-sm); overflow: hidden; }
  .table { margin-bottom: 0; }
  .table thead th {
    background: linear-gradient(135deg, var(--bhms-blue-light), #eaf2fb); color: var(--bhms-blue-dark);
    font-size: 0.72rem; text-transform: uppercase; font-weight: 600; border-bottom: none; padding: 0.85rem 1rem;
  }
  .table td { padding: 0.7rem 1rem; vertical-align: middle; font-size: 0.88rem; }
  .table-striped > tbody > tr:nth-of-type(odd) > * { background-color: var(--bhms-gray-50); }
  .badge-inactive { background: var(--bhms-gray-200); color: var(--bhms-gray-600); padding: 0.3em 0.7em; border-radius: 999px; font-size: 0.72rem; font-weight: 600; }
  .badge-active { background: var(--bhms-success-light); color: var(--bhms-success-darker); padding: 0.3em 0.7em; border-radius: 999px; font-size: 0.72rem; font-weight: 600; }
</style>
</head>
...
<div class="bhms-shell">
  ...
  <aside class="bhms-sidebar">
    ...
    <nav class="bhms-nav">
      ...
      <a href="../admin/user_management.php" class="bhms-nav-link active"><i class="fa-solid fa-user-gear"></i><span>User Management</span></a>
  ...
  <div class="bhms-main">
    <header class="bhms-topbar">
      ...
      <div class="bhms-topbar-title">User Management</div>
      <div class="bhms-topbar-user">
        <i class="fa-regular fa-circle-user"></i>
        <span><?= htmlspecialchars($_SESSION['full_name']) ?></span>
        <span class="bhms-role-pill">Administrator</span>
      </div>
    </header>
    <main class="bhms-content">
<div class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h3><i class="fa-solid fa-user-gear me-2" style="color:var(--bhms-blue);"></i>User Management</h3>
  ...
  <div class="card mb-4">
    <div class="card-body">
      <h5 class="card-title"><i class="fa-solid fa-user-plus me-2"></i>Add new user</h5>
      <form method="POST" action="">
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <input type="password" name="new_user_password" class="form-control" required minlength="8">
          </div>
        </div>
        <button type="submit" name="add_user" class="btn btn-primary mt-3"><i class="fa-solid fa-plus me-2"></i>Add user</button>
      </form>
    </div>
  </div>

  <h5><i class="fa-solid fa-list-check me-2"></i>All accounts</h5>
  ...
  <table class="table table-striped">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= $u['role'] === 'administrator' ? 'Administrator' : 'BHW' ?></td>
        <td><span class="<?= $u['is_active'] ? 'badge-active' : 'badge-inactive' ?>"><?= $u['is_active'] ? 'Active' : 'Inactive' ?></span></td>
        <td>
          <button type="button" class="btn btn-sm btn-outline-warning" data-userid="<?= $u['user_id'] ?>" data-username="<?= htmlspecialchars($u['username'], ENT_QUOTES) ?>" onclick="openResetModal(this)">Reset Password</button>
          <a href="?toggle=<?= $u['user_id'] ?>" class="btn btn-sm <?= $u['is_active'] ? 'btn-outline-secondary' : 'btn-outline-success' ?>" onclick="return confirm('<?= $u['is_active'] ? 'Deactivate' : 'Reactivate' ?> this account?')"><?= $u['is_active'] ? 'Deactivate' : 'Reactivate' ?></a>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-body p-4">
        <h5 class="mb-3"><i class="fa-solid fa-key me-2" style="color:var(--bhms-blue);"></i>Reset password for <span id="resetModalUsername"></span></h5>
        <form method="POST" action="">
...
function openResetModal(btn) {
    ...
    document.getElementById('resetModalUsername').innerText = btn.dataset.username;
