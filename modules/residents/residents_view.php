<?php
/** @var array|null $edit_resident */
/** @var string $error */
/** @var array $residents */
/** @var string $success */
if (!isset($residents)) { return; }
?>
<!DOCTYPE html>
<html lang="en">  
<head>
...
<style>
  ...
  /* ---- Sidebar navigation scrollbar: transparent, brightens on hover ---- */
...
.bhms-nav::-webkit-scrollbar-button:vertical:end:increment {
  ...
  .btn-outline-success { color: #2F6FE0; border-color: #2F6FE0; }
  .btn-outline-success:hover { background: #2F6FE0; border-color: #2F6FE0; }
  ...
  .resident-name-cell { display: flex; align-items: center; gap: 0.6rem; }
  .resident-icon {
...
<div class="bhms-shell">
  ...
  <aside class="bhms-sidebar">
    ...
    <nav class="bhms-nav">
      ...
      <a href="../residents/residents.php" class="bhms-nav-link active"><i class="fa-solid fa-users"></i><span>Resident Profiling</span></a>
  ...
  <div class="bhms-main">
    <header class="bhms-topbar">
      ...
      <div class="bhms-topbar-title">Resident Profiling</div>
...
<div class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h3><i class="fa-solid fa-users me-2" style="color:var(--bhms-blue);"></i>Resident Profiling</h3>
      ...
      <form method="POST" action="" id="residentForm">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <input type="text" name="contact_number" class="form-control" pattern="09[0-9]{9}" maxlength="11" inputmode="numeric" placeholder="09XXXXXXXXX" title="Enter an 11-digit mobile number starting with 09" value="<?= htmlspecialchars($edit_resident['contact_number'] ?? '') ?>">
          </div>
          <div class="col-md-12">
    ...
    <tbody>
      ...
      <tr data-gender="<?= htmlspecialchars($r['gender']) ?>" data-purok="Purok <?= htmlspecialchars($r['purok']) ?>" data-vital="<?= htmlspecialchars($r['vital_status']) ?>">
        ...
        <td>
          ...
          onclick="showQr('<?= htmlspecialchars($r['qr_code'], ENT_QUOTES) ?>', '<?= htmlspecialchars($r['first_name'] . ' ' . $r['last_name'], ENT_QUOTES) ?>')">View QR</button>
